...
# The default in Sage is cdivision=True, but we have to set it to
# False.  Do cdivision_warnings=True to see at runtime how Python
# division semantics are used all over the place in this file.
# 
...
This module defines fast Cython code needed to compute Hilbert modular
forms over F = Q(sqrt(5)).
...
def ResidueRing(P, unsigned int e):
    """
    Create the residue class ring $O_F/P^e$, where $P$ is a prime
    ideal of the ring $O_F$ of integers of Q(sqrt(5)).
    ...
    EXAMPLES::
        ...
        sage: from sage.modular.hilbert.sqrt5_fast  import F, ResidueRing
        sage: R = ResidueRing(F.prime_above(31), 3); R
        Residue class ring of (5*a - 2)^3 of characteristic 31
        sage: type(R)
        <type 'sage.modular.hilbert.sqrt5_fast.ResidueRing_split'>
        sage: R = ResidueRing(F.prime_above(5), 2); R
        Residue class ring of (-2*a + 1)^1 of characteristic 5
        sage: type(R)
        <type 'sage.modular.hilbert.sqrt5_fast.ResidueRing_nonsplit'>
        sage: list(R)
        [0, 1, 2, 3, 4, g, 1 + g, 2 + g, 3 + g, 4 + g, 2*g, 1 + 2*g, 2 + 2*g, 3 + 2*g, 4 + 2*g, 3*g, 1 + 3*g, 2 + 3*g, 3 + 3*g, 4 + 3*g, 4*g, 1 + 4*g, 2 + 4*g, 3 + 4*g, 4 + 4*g]
        sage: R = ResidueRing(F.prime_above(5), 1); R
        Residue class ring of (-2*a + 1)^1 of characteristic 5
        sage: type(R)
        <type 'sage.modular.hilbert.sqrt5_fast.ResidueRing_ramified_odd'>
        sage: R = ResidueRing(F.prime_above(3), 2); R
        Residue class ring of (3)^2 of characteristic 3
        sage: type(R)
        <type 'sage.modular.hilbert.sqrt5_fast.ResidueRing_nonsplit'>    
    """
    if e <= 0:
...
cdef class ResidueRingElement_nonsplit(ResidueRingElement):
    ...
    EXAMPLES::
        ...
        sage: from sage.modular.hilbert.sqrt5_fast import ResidueRing
...
cdef class ModN_Reduction:
    ...
    def __init__(self, N, bint init=True):
        ...
        if not (is_Ideal(N) and N.number_field() is F):
...
cdef class IcosiansModP1ModN:
    ...
    EXAMPLES::
        ...
        sage: from sage.modular.hilbert.sqrt5_fast  import F, IcosiansModP1ModN
        ...
        sage: from sage.modular.hilbert.sqrt5_tables import ideals_of_bounded_norm
    ...
    def _hecke_matrix_badnorm(self, P, sparse=True):
        ...
        EXAMPLES::
            ...
            sage: from sage.modular.hilbert.sqrt5_fast import F, IcosiansModP1ModN; v=F.primes_above(71); I=IcosiansModP1ModN(v[0])
